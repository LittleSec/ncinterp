...
for key in keyList:
	var = f.variables[key]
	observedValue = var.data[:,latIndex1:latIndex2,lonIndex1:lonIndex2].copy()
	observedValue = np.where(observedValue == var._FillValue, nanValue, observedValue.astype(float) * var.scale_factor + var.add_offset)
	header.append(key)
